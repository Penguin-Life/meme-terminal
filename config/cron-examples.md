...
openclaw cron add \
  --schedule "0 9 * * *" \
  --tz "Asia/Shanghai" \
  --prompt "Run morning meme terminal scan. Check http://localhost:3902/api/health first. Then GET /api/token/trending and POST /api/analyze/market. Send Taxy a morning briefing on Telegram with top 3 trending + market sentiment."
